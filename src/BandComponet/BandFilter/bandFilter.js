...
import api, { Band } from "../../webApi/Home/api";
import { toast } from "react-toastify";

function BandFilter({setBandList,setIsLoading,setError,getBand,bandList}){
    ...
    function debounce(func, timeout = 1000) {
      ...
      try{
         if(searchKey.current.value){
          ...
          setIsLoading(true)
           var response= await axios.get(baseUrl.BASE_URl+Band.SEARCH_BAND+"/"+searchKey.current.value);
          
           if(response.status)
           {
            if(!response.data.bandList.length==0){
      ...
      const byCharges = async (first,second) => {
        try {
          setIsLoading(true)
            var response = await axios.post(baseUrl.BASE_URl + Band.GETBAND_BYCHARGES, { firstPrice:first,secondPrice:second});
            if (response.status) {
              setBandList(response.data.bandList)
              setIsLoading(false);
          }
      ...
      const byServices= async (service) => {
        try {
          setIsLoading(true)
            var response = await axios.post(baseUrl.BASE_URl + Band.GETBAND_BY_SERVICES, {serviceName:service});
            if (response.status) {
              setBandList(response.data.bandList)
              setIsLoading(false);
          }
   ...
   const getService=()=>{
    bandList.map((band,index)=>{
      ...
      band.services.map((service,index)=>{
