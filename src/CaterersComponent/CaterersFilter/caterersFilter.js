...
import api, { Caterers, Mehandi } from "../../webApi/Home/api";
import { toast } from "react-toastify";

function CaterersFilter({setCaterersList,setIsLoading,setError,getCaterers,caterersList}){
    ...
    function debounce(func, timeout = 1000) {
      ...
      try{
         if(searchKey.current.value){
          ...
          setIsLoading(true)
           var response= await axios.get(baseUrl.BASE_URl+Caterers.SEARCH_CATERERS+"/"+searchKey.current.value);
        
           if(response.status)
           {
            if(!response.data.caterersList.length==0){
            setCaterersList(response.data.caterersList);
            setIsLoading(false);
            }
            else{
      ...
      const byCharges = async (first,second) => {
        try {
          setIsLoading(true)
            var response = await axios.post(baseUrl.BASE_URl + Caterers.GETCATERERS_BY_CHARGES, { firstPrice:first,secondPrice:second});
            if (response.status) {
              setCaterersList(response.data.caterersList)
              setIsLoading(false);
          }
      ...
      const bySerives= async (service) => {
        try {
          setIsLoading(true)
            var response = await axios.post(baseUrl.BASE_URl + Caterers.GETCATERERS_BY_SERVICES, {serviceName:service});
            if (response.status) {
              setCaterersList(response.data.caterersList)
              setIsLoading(false);
          }
  ...
  const getService=()=>{
    caterersList?.map((caterers,index)=>{   
      caterers?.services.map((service,index)=>{
