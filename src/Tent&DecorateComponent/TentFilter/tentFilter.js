...
import api, { Mehandi, Tent } from "../../webApi/Home/api";
import { toast } from "react-toastify";

function TentFilter({setTentList,setIsLoading,setError,getTent,tentList}){
    ...
    function debounce(func, timeout = 1000) {
      ...
      try{
         if(searchKey.current.value){
          ...
          setIsLoading(true)
           var response= await axios.get(baseUrl.BASE_URl+Tent.SEARCH_TENT+"/"+searchKey.current.value);
        
           if(response.status)
           {
            if(!response.data.tentList.length==0){
      ...
      const byCharges = async (first,second) => {
        try {
          setIsLoading(true)
            var response = await axios.post(baseUrl.BASE_URl + Tent.TENT_BY_CHARGES, { firstPrice:first,secondPrice:second});
            if (response.status) {
              setTentList(response.data.TentList)
              setIsLoading(false);
          }
      ...
      const bySerives= async (service) => {
        try {
          setIsLoading(true)
            var response = await axios.post(baseUrl.BASE_URl + Tent.TENT_BY_SERVICES, {serviceName:service});
            if (response.status) {
              setTentList(response.data.tentList)
              setIsLoading(false);
          }
